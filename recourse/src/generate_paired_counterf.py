...
if __name__=="__main__":
    ...
    if basic_config['explainer'] == 'FACE':
        badX = X[(y == 0).flatten()]
        goodX = X[(y == 1).flatten()]
        X = torch.tensor(np.vstack([badX[:1000], goodX[:1000]]))
        y = torch.tensor(np.vstack([np.zeros(1000).reshape(-1, 1), np.ones(1000).reshape(-1, 1)]))
    ...
    for idx_ in ids:
        ...
        if basic_config['explainer'] == "DiCE":
            ...
            if counterfactuals.shape[0] == 0 or fake_counterfactuals.shape[0] == 0:
                print("yeet")
                continue
        ...
        out = np.hstack( [
            ...
            np.argmax(c_predictions,axis=1).reshape(-1, 1) ] )
        print(out)
        
        fake_out = np.hstack( [
            np.vstack( [ el.flatten() for el in fake_cf_decode ] ),
            np.argmax(fake_c_predictions,axis=1).reshape(-1, 1) ] )
            
        desired_label = data.features[ data.target_column ].decode( desired_output.reshape(1,-1) )
        desired_label = desired_label.squeeze()
        
        idxs = np.where( np.argmax(c_predictions,axis=1).reshape(-1, 1) == desired_label )[0]
        out = out[ idxs, : ]
        print(out)

        fake_idxs = np.where( np.argmax(fake_c_predictions,axis=1).reshape(-1, 1) == desired_label )[0]
        ...
        for col in data.columns:
            real_recourse[col] = np.abs(df[col] - df_ref[col])
            fake_recourse[col] = np.abs(fake_df[col] - fake_df_ref[col])
